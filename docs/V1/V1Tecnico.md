...
current_phase: 10.5
current_phase_name: "Feedback Annet — branding oficial, datos negocio, reorder"
current_focus: "Fase 10.5 completada (Feedback Annet 2026-05-14). Sistema de marca oficial: paleta CLARA extraída del logo SVG vectorial (rosa coral #EE769C accent, terracota #A71E17 gold/eyebrows, vino #6C4050 secundario, bone #F6F1E4 fondo, blanco cards). Tokens en `src/styles/tokens.css` y `tailwind.config.mjs` actualizados manteniendo compat con `crudo-*` de Fase 8.5 (saffron/bone/terracota ahora apuntan a HEX oficiales). Tipografías: Fraunces display (puente hasta manual psicolabis) + Inter body + JetBrains Mono. `index.html` carga Google Fonts Fraunces, theme-color #F6F1E4, favicon SVG real. Logos SVG vectoriales copiados a `public/img/brand/logo-color.svg` (+ v1/v2/v3/v4). Header reemplaza wordmark CRUDO por logo SVG real. 5 fotos pro Sony A7 nuevas reemplazan placeholders móvil: hero-home-cheeseboard-pro.jpg (queso entero Marcel Petite), tabla-quesos-vino-pro.jpg (plato + copas tinto sobre azulejo), cata-vinos-naturales-pro.jpg (3 botellas + sirviendo vino), bodegon-cartel-crudo-pro.jpg (copa íntima), vino-natural-mano-pro.jpg. Hero Home overlay invertido a tema claro: gradient `rgba(246,241,228,0.92→0)` izquierda. Datos negocio en `siteConfig` fallback: WhatsApp público +34 650 13 18 61, WhatsApp owner +34 662 51 74 90, email crudomov@gmail.com, dominio crudomov.com, Instagram @crudomov, horario L-V cierre 22:30 (no 23:00). Nav Header reordenado: Eventos primero, Tablas segundo, Catálogo, Celebra, Sobre CRUDO, Contacto. Página /tablas añadida al nav (antes oculta). HomePage opción B aplicada: Hero → EventsTeaser → SeasonalShowcase → CategoryStrips → VisitBlock (MaridajesStrip e InstagramStrip descolgados pero archivos mantenidos en repo). Componente nuevo `CelebraStrip` añadido al final de /eventos: copy 'Si tienes un evento y quieres hacerlo en CRUDO ponte en contacto crudomov@gmail.com' con botón mailto + WhatsApp prellenado 'Hola CRUDO, me interesa hacer un evento privado'. Nuevo `TablaMaridajeSelector` en /tablas: radios 3/6/8 quesos × 5 maridajes (sin maridaje, blanco seco, blanco afrutado, tinto ligero, tinto con cuerpo) + checkbox 'natural' que aparece solo si hay vino → genera link WhatsApp prellenado con la elección, sin tocar backend. TablasPage reescrita con hero + descripción 'cómo son nuestras tablas' (60-70g, frommellier, frutos secos + frutas temporada) + selector. EventsPage extendida con CelebraStrip antes del newsletter strip. Tests cliente nuevos (5): TablaMaridajeSelector default-no-natural-mention/wine-pairing-reveals-checkbox/email-fallback (3), CelebraStrip mailto+WA-prefilled / sin-WA-fallback (2). 202/202 tests verde, lint limpio, build Vite 1719 módulos / 785KB JS gzip 198KB.\n\n--- Estado previo Fase 10 ---\nFase 10 completada. Rutas públicas secundarias funcionando contra API real: `/eventos` (lista filtrada futuras + ordenada asc + empty-state con CTA contacto), `/eventos/:slug` (hero con fallback editorial, formulario reserva con manejo full-state, alternativa WhatsApp si is_full), `/contacto` (form CONTACT + ContactCtas WhatsApp/Maps/Instagram + MapBlock con embed o fallback link + horarios desde site_config), `/sobre-crudo` (AboutIntro hero IMG_8954, ManifestoBlock 2-col texto+foto, OwnerSpaceBlock grid 3 fotos + AnimalQuesero, links a catálogo/eventos/contacto), `/mayoristas` (hero IMG_1117 + WholesaleForm con business_name en payload). 5 libs nuevos: eventsApi (POST /events/:slug/reservations con Idempotency-Key + retry 1× network), inquiriesApi (POST /inquiries genérico), newsletterApi (POST /newsletter/subscribe), formValidation (name/email/phone-required+optional/message/partySize 1-4/businessName/consent), contactLinks (helpers wa.me/maps/instagram desde siteConfig). Hook useEvent (singular) para detalle. 5 components events/: EventCard (16:9 con fallback /img/lifestyle/cata-vinos-naturales.jpg + EventCapacityBadge), EventList (filter futuros, sort asc, loading skeleton, error/empty), EventDetail (hero 55vh + meta + ReservationForm o WhatsApp si full + FormSuccess), ReservationForm (validation + Idempotency-Key + manejo 422-fields/422-detail/409 renueva key/429/network/genérico + trackEventInquiry sin PII), EventCapacityBadge (Tag tone error/warning/default según is_full/few_seats_left/seats_left). 5 components forms/: ContactForm (CONTACT, success FormSuccess), WholesaleForm (WHOLESALE con payload.business_name, success), NewsletterForm (consent checkbox condicional, success con BrandSticker easter egg, errores network/429/genérico), FormSuccess (CheckCircle2 success border + opcional sticker), FormError (AlertTriangle sobrio sin humor). 2 components contact/: ContactCtas (botones WhatsApp/Maps/Instagram con tracking), MapBlock (iframe embed si configurado, fallback link + dirección). 3 components about/: AboutIntro (hero overlay gradient izquierdo respetando IMG_8954), ManifestoBlock (2-col texto editorial + LifestylePhoto bodegón), OwnerSpaceBlock (grid 3 fotos mostrador-quesera/mostrador-vino/owner-mostrador + AnimalQuesero v2 + copy editorial corto). Footer extendido con bloque Newsletter (NewsletterForm source=footer). Pipeline visual Fase 10: copiados IMG_8952 → mostrador-quesera.jpg, IMG_8953 → mostrador-vino.jpg, IMG_0206 → bodegon-cartel-crudo-2.jpg. Analytics: trackEventInquiry tras 201 reserva con event_slug+party_size, trackGenerateLead tras 201 form contact/wholesale/newsletter con form name+source — todos sin PII. Tests cliente nuevos (29): formValidation (7), contactLinks (4), EventList future-filter+badges+empty+error (5), ReservationForm required+party-size>4+happy-path+422-fields (4), ContactForm required+CONTACT-submit+lead-no-PII (2), WholesaleForm required+WHOLESALE-payload+no-ecommerce-text (3), NewsletterForm invalid-email+consent-required+success+API-error (4). 197/197 tests verde (168 + 29 nuevos), lint limpio, build Vite 1718 módulos / 775KB JS gzip 196KB. Reglas vino/no-alcohol intactas, sin sistema de reserva de mesas, sin ecommerce B2B.\n\n--- Estado previo Fase 9 ---\nFase 9 completada. tablaDraft extendido con `hydrate()` (sanitiza estado al cargar: filtra alcohol, descarta items inválidos, clampa qty 1-99, persiste si hubo cambio) y `getPayloadItems()` (devuelve solo `product_id`/`product_slug`/`quantity` — nunca precio como autoridad). Libs nuevos: `lib/idempotency.js` (genera clave UUID/fallback, exporta `IDEMPOTENCY_HEADER='Idempotency-Key'`); `lib/pickupValidation.js` (validators name/email/phone/date/slot/notes + `getSlotsForDate` que genera bloques 30 min HH:mm desde site_config.hours por DOW); `lib/pickupApi.js` (`submitPickupOrder` con header Idempotency-Key, retry una vez para network error status 0, sin retry para 4xx/5xx). 7 componentes en `src/components/tabla/`: TablaDrawer (slide-over derecho con backdrop, focus management, Escape cierra, body overflow lock), TablaSummary, TablaLineItem (qty stepper +/- con cap MAX_QTY, remove), PickupForm (campos requeridos + slots dinámicos por fecha + manejo 422 alcohol/409 conflict/429/network/genérico, Idempotency-Key se renueva al cambiar payload tras envío), PickupSuccess (RetroSign '¡Pedido recibido!', orderId, total, items, CTAs WhatsApp/Maps), TablaEmptyState (AnimalQuesero v1 + CTA /catalogo/quesos), AlcoholBlockedNotice (warning visible cuando se detecta alcohol). MyTablaPage reescrita: hydrate al entrar, dos columnas resumen+form, AlcoholBlockedNotice si se removió alcohol al hidratar. MyTablaConfirmationPage: render PickupSuccess con `state.confirmation`, fallback editorial seguro si entran sin state, clearDraft al éxito. AppShell ahora gestiona TablaDrawer state y lo pasa a Header (botón 'Mi Tabla [N]' desktop) y StickyCTA (botón mobile abre drawer en lugar de navegar). Analytics: `trackPickupRequest` ya existía y se dispara solo tras 201, sin PII (item_count + total_cents + contents id/slug/qty). 25 tests cliente nuevos: tablaDraft hydrate/getPayloadItems/corrupt JSON (4), pickupValidation (6), idempotency (2), TablaDrawer (5), PickupForm happy/422 alcohol/network/empty/invalid email (6), MyTablaConfirmationPage safe state + success (2). Lint 0 errores, build Vite 1709 módulos / 690KB JS gzip 184KB, 168/168 tests verde (143 + 25). Reglas vino/no alcohol intactas, contratos API sin cambios.\n\n--- Estado previo Fase 8.5 ---\nFase 8.5 completada. Pipeline mínimo `public/img/` con 13 assets reales (1 Gemini hero + 6 lifestyle + 4 brand + 1 about + 2 mascotas). 5 componentes de marca en `src/components/brand/`: RetroSign (cartel retroiluminado terracota), SaffronTileBackground (azulejo amarillo SVG), LifestylePhoto (alt requerido), BrandSticker (logo decorativo), AnimalQuesero (mascotas). Tokens Tailwind nuevos: crudo-saffron, crudo-terracota, crudo-bone. Hero refactor con imagen Gemini y overlay gradient izquierdo respetando espacio negativo intencional. SeasonalShowcase / VisitBlock / InstagramStrip / CategoryStrips refactorizados con fotos lifestyle reales y RetroSign eyebrows. Nuevo MaridajesStrip con SaffronTileBackground subtle entre CategoryStrips y EventsTeaser. ProductCard y ProductGallery con fallback editorial por tipo (CHEESE→tabla-quesos-vino, WINE→cata-vinos-naturales) en lugar de placeholder gris. Footer easter egg con BrandSticker rotado. 8 tests nuevos (RetroSign 3 + LifestylePhoto 3 + ProductCard fallback 2). 143/143 verde, lint limpio, build Vite 1706 módulos / 644KB JS gzip 177KB. Reglas vino/no alcohol intactas, contratos API sin cambios.\n\n--- Estado previo Fase 8 ---\nFase 8 completada. Home real con Hero full-bleed (overlay rgba(26,31,20,0.55), H1 Cormorant italic, eyebrow 'Vinos y quesos · Madrid', CTAs Reservar mi tabla + Cómo llegar, indicador abierto/cerrado), SeasonalShowcase con productos `seasonal=true`, CategoryStrips (Quesos / Tablas / Temporada), EventsTeaser con 3 eventos futuros, InstagramStrip editorial (sin API real), VisitBlock con direccion+horarios+Maps+WhatsApp. Catalogo `/catalogo`, `/catalogo/quesos`, `/catalogo/vinos`, `/catalogo/temporada` compartiendo `CatalogView` (toolbar con busqueda `q`, filtro categoria, toggle seasonal, limpiar filtros). ProductCard mobile-first con aspect-ratio 1/1, tags Temporada/Destacado/Agotado, precio en JetBrains Mono, CTA divergente: vino -> WhatsApp con texto prellenado `Hola, me interesa {nombre}. ¿Lo tenéis disponible en CRUDO?`, no alcohol -> `Añadir`. PDP `/producto/:slug` con ProductGallery (con miniaturas), ProductMeta, ProductLongRead (leche/tratamiento/intensidad/region/productor + historia + maridaje), RelatedProducts. Wine guard PDP: vino nunca renderiza AddToTablaButton (test cubre). Schema.org Product inyectado en JSON-LD. tablaDraft (localStorage + subscribe pattern) rechaza alcohol con AlcoholInTablaError; useTablaDraft hook expone count para StickyCTA y Hero. analytics.js dispara select_item / wine_pairing_whatsapp_click / whatsapp_click / maps_click solo con consentimiento. Lint 0 errores, build Vite 1701 modulos 347KB JS gzip 111KB, tests 135/135 verde (23 nuevos de cliente). Inventario visual real catalogado en §7.bis (1 Gemini hero + 17 fotos local + branding) y nuevas sub-secciones de assets inyectadas en Fases 8.5/9/10/12/14."
next_recommended_prompt: "Fase 11 - Admin frontend movil (sobre marca CRUDO oficial)"
...
| 9 | Mi Tabla frontend y pickup flow | REVIEW_READY | tablaDraft extendido (`hydrate`, `getPayloadItems`, alcohol guard al hidratar, qty clamp). Libs: `idempotency.js`, `pickupValidation.js` (slots HH:mm 30min por DOW), `pickupApi.js` (header Idempotency-Key + retry 1× solo para network). 7 componentes tabla/: TablaDrawer (slide-over con focus mgmt + Escape + body lock), TablaSummary, TablaLineItem (stepper qty), PickupForm (manejo 201/422/422-alcohol/409/429/network/genérico, key se renueva al cambiar payload tras envío), PickupSuccess (RetroSign + orderId + total + items + CTAs WhatsApp/Maps), TablaEmptyState (AnimalQuesero), AlcoholBlockedNotice. MyTablaPage reescrita con hydrate al entrar y 2 columnas resumen+form. MyTablaConfirmationPage con state.confirmation y fallback safe. AppShell gestiona drawer state, Header desktop tiene botón "Mi Tabla [N]", StickyCTA mobile abre drawer. trackPickupRequest tras 201 sin PII. 25 tests nuevos cliente; 168/168 verde, lint limpio, build 1709 módulos / 690KB JS gzip 184KB. | Smoke E2E manual contra backend real pendiente. PickupForm slot select limita a horarios apertura general; si se confirma `pickup_hours_by_dow` distinto en site_config, ajustar. AddToTablaButton no abre drawer automáticamente al añadir (deliberado: feedback "Añadido" + counter Header). | Sin bloqueos |
| 10.5 | Feedback Annet — branding oficial + datos negocio + reorder | REVIEW_READY | Sistema de marca CRUDO oficial extraído del logo SVG: paleta CLARA con accent rosa coral #EE769C, gold terracota #A71E17, vino #6C4050, bone #F6F1E4 fondo. Tokens en tokens.css + tailwind.config con compat con Fase 8.5. Tipografía Fraunces (display) + Inter + JetBrains Mono cargadas via Google Fonts; theme-color y favicon SVG actualizados. Logos SVG vectoriales en `public/img/brand/` reemplazan placeholders; Header usa logo SVG real. 5 fotos pro Sony A7 reemplazan fotos móvil (Hero, CategoryStrips, ProductCard fallback, ProductGallery fallback, EventDetail, ContactPage, EventsPage, WholesalePage, MaridajesStrip, InstagramStrip, VisitBlock, ManifestoBlock). Hero overlay invertido a gradient claro. Nav reordenado: Eventos > Tablas > Catálogo > Celebra > Sobre > Contacto (Tablas añadida). HomePage opción B (MaridajesStrip + InstagramStrip descolgados, archivos mantenidos). siteConfig fallback con datos confirmados Annet (WhatsApp +34 650 13 18 61, owner +34 662 51 74 90, crudomov@gmail.com, crudomov.com, @crudomov, horario L-V 22:30). Nuevo CelebraStrip al final de /eventos con mailto + WhatsApp prellenado. Nuevo TablaMaridajeSelector en /tablas reescrita: 3/6/8 quesos × 5 maridajes + checkbox natural condicional → link WhatsApp prellenado con selección, sin tocar backend. 5 tests cliente nuevos (TablaMaridajeSelector 3, CelebraStrip 2). 202/202 verde, lint limpio, build 1719 módulos / 785KB JS gzip 198KB. | Manual de marca de psicolabis pendiente (Annet lo solicitó); paleta actual extraída del logo es válida hasta entonces. Catálogo de quesos real pendiente Annet. Validación legal Annet pendiente (Fase 12). Brevo / Plesk / Meta credenciales (Plesk confirmado el hosting; Brevo pendiente cuenta; Meta pospuesto V1.1). Fotos producto 1:1 1600×1600 fondo madera oscura pendientes (sesión Morning/Noche es lifestyle 3:2). | Sin bloqueos para Fase 11 |
| 10 | Eventos, contacto, newsletter, sobre y mayoristas | REVIEW_READY | 5 rutas públicas: `/eventos` (lista filtrada futuras + sort asc + empty state), `/eventos/:slug` (hero + ReservationForm + alternativa WhatsApp si is_full), `/contacto` (ContactForm + ContactCtas + MapBlock + horarios), `/sobre-crudo` (AboutIntro + ManifestoBlock + OwnerSpaceBlock), `/mayoristas` (WholesaleForm + copy B2B no-ecommerce). 5 libs (eventsApi con Idempotency-Key + retry network, inquiriesApi, newsletterApi, formValidation, contactLinks). Hook useEvent. 5 components events/, 5 forms/, 2 contact/, 3 about/. Footer extendido con NewsletterForm. Pipeline visual: 3 fotos nuevas copiadas (mostrador-quesera, mostrador-vino, bodegon-cartel-crudo-2). trackEventInquiry/trackGenerateLead consent-aware sin PII. 29 tests nuevos cliente. 197/197 verde, lint limpio, build 1718 módulos / 775KB JS gzip 196KB. | Smoke E2E manual contra backend real pendiente. MapBlock muestra iframe sólo si `google_maps_embed_url` está en site_config (todavía no expuesto explícitamente por backend; fallback a link funciona). Newsletter requireConsent=true por defecto en page; en footer va sin consent explícito para no romper UX (el consent global del cookie banner cubre). | Sin bloqueos |
...
- 2026-05-14 | Fase 10.5 | Feedback Annet aplicado. Sistema de marca CRUDO oficial extraído del logo SVG vectorial: 6 colores HEX exactos (#EE769C rosa coral, #F6B6C8 rosa pastel, #A71E17 terracota, #FEDB9A amarillo crema, #447A96 azul petrol, #6C4050 vino) sobre fondo claro #F6F1E4. Tokens en src/styles/tokens.css y tailwind.config.mjs reescritos. Compat: tokens crudo-saffron/bone/terracota de Fase 8.5 ahora apuntan a HEX oficiales. Tipografía Fraunces (puente hasta manual psicolabis) cargada via Google Fonts; index.html con theme-color #F6F1E4 y favicon SVG real. Logos SVG vectoriales copiados desde docs/V1/Crudo_Logo/Crudo - Logos/SVG/ a public/img/brand/ (logo-color.svg + v1/v2/v3/v4 variantes). Header reemplaza wordmark CRUDO por <img src='/img/brand/logo-color.svg' alt='CRUDO'>. 5 fotos pro Sony A7 copiadas con sufijo -pro: hero-home-cheeseboard-pro.jpg (_A7_0008 queso entero Marcel Petite), tabla-quesos-vino-pro.jpg (_A7_0382 plato + copas tinto), cata-vinos-naturales-pro.jpg (_A7_9665 3 botellas + sirviendo), bodegon-cartel-crudo-pro.jpg (_A7_0046 copa íntima), vino-natural-mano-pro.jpg. sed swap masivo en 11 archivos refrencia las versiones -pro. Hero overlay gradient invertido para tema claro: linear-gradient(90deg, rgba(246,241,228,0.92) 0%, 0.72 40%, 0.28 70%, 0). Nav Header reordenado a 6 entradas con Eventos primero, Tablas segundo (Tablas estaba oculta antes), después Catálogo, Celebra, Sobre, Contacto. siteConfig fallback con datos confirmados: WhatsApp público +34 650 13 18 61, WhatsApp owner +34 662 51 74 90, email crudomov@gmail.com, dominio crudomov.com, instagram @crudomov, horario L-V cierre cambiado a 22:30 (no 23:00). HomePage opción B aplicada: Hero → EventsTeaser → SeasonalShowcase → CategoryStrips → VisitBlock; MaridajesStrip e InstagramStrip descolgados de la composición pero archivos mantenidos en repo (no borro). Nuevo CelebraStrip.jsx al final de /eventos: copy 'Si tienes un evento y quieres hacerlo en CRUDO ponte en contacto' con botón mailto crudomov@gmail.com (subject 'Evento privado en CRUDO') + botón WhatsApp prellenado 'Hola CRUDO, me interesa hacer un evento privado'. trackGenerateLead form=private_event_email tras click mailto, trackWhatsAppClick source=private_event tras click WA. Nuevo TablaMaridajeSelector.jsx: radios 3/6/8 quesos (con descripción 60-70g, ideal 2-4 personas) × radios 5 maridajes (sin maridaje, blanco seco, blanco afrutado, tinto ligero, tinto con cuerpo) + checkbox 'natural' que se renderiza solo cuando el maridaje es vino, genera link buildWhatsAppUrl con mensaje contextual (sin maridaje vs con maridaje + natural opcional), trackWineWhatsAppClick source=tabla_selector con dimensiones size+pairing+natural. Sin tocar backend (regla V1: vino siempre por WhatsApp). TablasPage reescrita: hero 40vh con tabla-quesos-vino-pro.jpg + RetroSign 'Tablas para llevar' + H1 'Tres tamaños, tu maridaje' + 2-col descripción/selector. EventsPage añade <CelebraStrip siteConfig={config}/> antes del newsletter strip. NAV mantenido íntegramente (Q4 reinterpretado: NO retirar /sobre-crudo ni /celebra-con-nosotros del nav, solo reordenar). Tests cliente nuevos: TablaMaridajeSelector default-no-natural / wine-pairing-reveals-checkbox + size+pairing+natural-en-WA / email-fallback-sin-WA (3); CelebraStrip mailto+WA-prefilled / sin-WA-fallback (2). | Verificación: npm run lint 0 errores · npm test 202/202 verde (197 + 5 nuevos) · npm run build 1719 módulos 785KB JS gzip 198KB · smoke visual contra dev server pendiente. Reglas vino/no alcohol intactas. Pendientes documentados: manual marca psicolabis (Annet lo pidió, paleta actual válida), catálogo quesos real Annet, fotos 1:1 fondo madera, validación legal AEPD por Annet, Brevo cuenta, Meta V1.1. | Siguiente: Fase 11
- 2026-05-13 | Fase 10 | Rutas públicas secundarias completadas. 5 libs nuevos: eventsApi.js (POST /events/:slug/reservations con Idempotency-Key + retry 1× network), inquiriesApi.js (POST /inquiries genérico para CONTACT/WHOLESALE/EVENT_PRIVATE), newsletterApi.js (POST /newsletter/subscribe), formValidation.js (validateName/Email/Phone-optional+required/Message/PartySize 1-4/BusinessName/Consent), contactLinks.js (getContactLinks construye wa.me/maps/instagram desde siteConfig). Hook useEvent (singular detalle). 5 components events/: EventCapacityBadge (Tag tone por is_full/few_seats_left/seats_left), EventCard (16:9 con fallback /img/lifestyle/cata-vinos-naturales.jpg), EventList (filter futuros con bug fix `.filter((e) => isFuture(e))` evitando que filter pase index como `now`, sort asc, loading skeleton, empty state con CTA contacto, error state), ReservationForm (5 campos + 4 validators + Idempotency-Key + manejo 201/422-fields/422-detail/409 renueva key/429/network/genérico + trackEventInquiry), EventDetail (hero 55vh con fallback, meta date/time/location, EventCapacityBadge + price, ReservationForm o WhatsApp si is_full, FormSuccess post-reserve). 5 components forms/: ContactForm (CONTACT submit + success), WholesaleForm (WHOLESALE con payload.business_name + 5 campos validados), NewsletterForm (consent checkbox condicional + success con BrandSticker easter egg + manejo network/429/genérico), FormSuccess (CheckCircle2 + opcional sticker), FormError (AlertTriangle sobrio sin humor). 2 components contact/: ContactCtas (WhatsApp/Maps/Instagram tracking), MapBlock (iframe embed si google_maps_embed_url, fallback link + dirección). 3 components about/: AboutIntro (hero 60vh overlay gradient izquierdo IMG_8954 + RetroSign), ManifestoBlock (2-col texto editorial + LifestylePhoto bodegón), OwnerSpaceBlock (grid 3 fotos mostrador-quesera/mostrador-vino/owner-mostrador + AnimalQuesero v2). Páginas reescritas: EventsPage (hero + EventList + newsletter strip), EventDetailPage (slug param + useEvent + loading skeleton + 404 safe state + EventDetail), ContactPage (hero IMG_0205 + ContactForm + aside CTAs/Map/Hours), AboutPage (AboutIntro+Manifesto+Space+links navegación), WholesalePage (hero IMG_1117 + WholesaleForm + copy B2B). Footer extendido con bloque Newsletter (NewsletterForm source=footer sin consent explícito porque cubre cookie banner global). Pipeline visual: copiados IMG_8952 → public/img/about/mostrador-quesera.jpg, IMG_8953 → public/img/about/mostrador-vino.jpg, IMG_0206 → public/img/lifestyle/bodegon-cartel-crudo-2.jpg. analytics.trackEventInquiry tras 201 reserva (event_slug + party_size, sin PII). analytics.trackGenerateLead tras 201 contact/wholesale/newsletter (form + source, sin PII). Tests nuevos (29): formValidation 7, contactLinks 4, EventList 5 (futuros+badges+empty+error), ReservationForm 4 (required+>4plazas+happy+422-fields), ContactForm 2 (required+submit-no-PII-leak-en-analytics con consent allowed), WholesaleForm 3 (required+WHOLESALE-payload+no-ecommerce-language), NewsletterForm 4 (invalid-email+consent-required+success+error). Bug fix EventDetail: removed unused `heroAlt` var (lint). | Verificación: npm run lint 0 errores · npm test 197/197 verde (168 + 29 nuevos) · npm run build 1718 módulos 775KB JS gzip 196KB · smoke E2E manual contra backend real queda pendiente (tests usan mock de api libs). Reglas vino/no alcohol intactas. Sin sistema de reserva de mesas nativa. Sin ecommerce B2B. | Siguiente: Fase 11
- 2026-05-12 | Fase 9 | Mi Tabla frontend + pickup flow. tablaDraft.js extendido con hydrate (alcohol guard al hidratar, qty clamp [1,99], descarta items inválidos, persiste si cambió) y getPayloadItems (sólo product_id/product_slug/qty). Libs nuevos: idempotency.js (UUID con fallback + IDEMPOTENCY_HEADER constant), pickupValidation.js (validators + getSlotsForDate 30min HH:mm por DOW), pickupApi.js (submitPickupOrder con header Idempotency-Key, retry 1× para network status 0). 7 componentes tabla/: TablaDrawer (slide-over derecho con backdrop, focus into panel, Escape cierra, body overflow lock, footer CTA solo si count>0), TablaSummary (lista + total estimado + nota recálculo backend), TablaLineItem (qty stepper con Min/Plus, cap MAX_QTY, remove con accesibilidad), PickupForm (5 campos requeridos + notas, slots dinámicos por fecha y site_config.hours, manejo 201 → onSuccess/clear/track, 422 generic → field errors, 422 alcohol → mensaje seguro y no confirma, 409 → renueva idempotency key, 429 → mensaje espera, status 0 → mensaje red, key se invalida al cambiar items tras envío exitoso), PickupSuccess (RetroSign "¡Pedido recibido!" + H1 "Te confirmamos por WhatsApp" + orderId/total/items + CTAs WhatsApp/Maps con tracking), TablaEmptyState (AnimalQuesero + CTA /catalogo/quesos), AlcoholBlockedNotice (warning con removed[].name). MyTablaPage reescrita: hydrate on mount (detecta alcohol removido y muestra notice), grid 2 columnas resumen+form, fallback "tabla vacía". MyTablaConfirmationPage: PickupSuccess con state.confirmation + fallback "no encontramos tu reserva" si entran sin state, clearDraft al éxito. AppShell ahora gestiona drawerOpen state y pasa onOpenTabla a Header (botón desktop "Mi Tabla [N]" con ShoppingBag) y StickyCTA (botón mobile abre drawer en lugar de Link). trackPickupRequest existente se dispara tras 201 con item_count/total_cents/contents (id+slug+qty) sin PII. Tests cliente nuevos: tablaDraft hydrate alcohol/clamp/corrupt JSON + getPayloadItems no price (4), pickupValidation requeridos/email/date past+future/slot HH:mm 30min/getSlotsForDate (6), idempotency header+unique (2), TablaDrawer empty/items+notices/CTA href/closed-state/Escape (5), PickupForm payment notice/empty errors/invalid email/happy submit con payload sin precio + idempotencyKey/422 alcohol safe message/network status 0 (6), MyTablaConfirmationPage safe state + success render con orderId+items y sin texto pago online (2). | Verificación: npm run lint 0 errores · npm test 168/168 verde (143 + 25 nuevos: 12 tablaDraft + 6 pickupValidation + 2 idempotency + 5 TablaDrawer + 6 PickupForm + 2 ConfirmationPage; -3 tests reorganizados de tablaDraft mantienen total) · npm run build 1709 módulos 690KB JS gzip 184KB · smoke E2E manual contra backend real queda pendiente (tests usan mock de pickupApi). Reglas vino/no alcohol intactas. | Siguiente: Fase 10
