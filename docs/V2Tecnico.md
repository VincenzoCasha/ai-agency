...
| 1 | Produccion/Plesk y deploy reproducible | REVIEW_READY | Verificar crudomov.es, env, DB, build, migraciones, SSL y smoke. (Codigo/docs listos: dominio alineado a crudomov.es, flujo deploy corregido npm ci→build→prune, document root y nodenv documentados, smoke.sh ampliado. Smoke de produccion PENDIENTE de ejecucion del owner.) |
