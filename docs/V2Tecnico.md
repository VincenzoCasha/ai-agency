...
current_phase: 9
current_phase_name: "E2E, accesibilidad, performance y QA"
current_focus: "Fase 9 ejecutada (REVIEW_READY) el 2026-06-08: suite E2E + a11y + presupuesto de performance + checklist de QA manual. Alcance acotado consensuado con owner: E2E con API MOCKEADA (Playwright intercepta /api/v1/** via page.route; sin Express ni MariaDB -> rapido, determinista, CI-friendly) y a11y/perf integrados (axe + script de bundle), SIN Lighthouse CI ni E2E contra backend real. Entregables nuevos: playwright.config.js (2 proyectos chromium: 'mobile' iPhone-viewport primario + 'desktop'; webServer=npm run dev:client; webkit NO instalado -> iPhone Safari real queda como QA manual); e2e/fixtures/mockApi.js (mock canonico por endpoint + helper hideCookieBanner que siembra consent en localStorage para que el banner fixed no intercepte clics); e2e/helpers/a11y.js (expectNoSeriousA11y con axe wcag2a/2aa/21a/21aa, falla en serious/critical; auditContrast no bloqueante) y e2e/helpers/layout.js (sin scroll horizontal + tap targets nav >=44px en movil); 8 specs (home, catalog con alcohol guard frontend, mi-tabla flujo completo pickup->confirmacion, eventos+reserva, contacto+newsletter, cookies aceptar/rechazar/configurar, legal, admin login+guard); scripts/check-bundle.mjs (gzip del chunk index-*.js <=160KB y public/img/v2 <=1.5MB, sin deps); docs/qa-checklist.md (Chrome/iPhone Safari/Instagram in-app + a11y teclado/zoom + hallazgo contraste). package.json: +test:e2e, +test:e2e:ui, +check:bundle. .github/workflows/pr.yml: +step check:bundle en el job build + nuevo job 'e2e' (chromium, sin DB) con upload del report. .gitignore: +playwright-report/test-results/blob-report. devDeps: @playwright/test, @axe-core/playwright. Alcohol guard cubierto: FRONTEND en E2E (vino->solo WhatsApp wa.me, nunca 'Añadir'; Mi Tabla sin alcohol) + BACKEND 422 ya en tests/ de servidor (referenciado, no re-testeado por estar mockeado). HALLAZGO (no bloqueante, para diseño): axe detecta ~23+ nodos con contraste < AA (acento rosa #ee769c sobre crema #f6f1e4 = 2.41) por toda la paleta de marca; regla color-contrast EXCLUIDA del fallo automatico (corregir = tocar tokens de diseño, fuera de alcance Fase 9) y auditada/reportada sin bloquear; pendiente decision de diseño (anotado en qa-checklist.md). Sin tocar codigo de la app (solo selectores accesibles ya existentes; 1 data-testid preexistente en CookieBanner). Verificado local: lint limpio, test:client 90/90, build OK sin warning, check:bundle OK (index 130.6KB gzip; img v2 1443KB), test:e2e 58/58 (mobile+desktop). Pendiente owner/futuro: QA manual real (iPhone Safari, Instagram in-app, Lighthouse mobile, teclado/lector); decision contraste; tests server con DB en CI (ya cubierto por pr.yml job build). Fases previas: 0,1,2,3,4,5,6,7,8.\nNOTA Fase 1 CERRADA (DONE) el 2026-06-08: deploy completo ejecutado en produccion crudomov.es por el owner via Plesk UI (sin SSH). Resultado: smoke.sh BASE_URL=https://crudomov.es -> ok=9 fail=0 (health, products, categories, events, site/config, admin/dashboard 401 sin token, SPA '/', robots.txt, sitemap.xml). Cronologia real del deploy (referencia para futuros redeploys sin SSH): (1) Plesk > Git > 'Pull now'+'Deploy now' trajo el commit 6526258 (Fase 1: deploy reproducible + dominio crudomov.es) a /httpdocs; el script de 'Additional deployment actions' (npm install && npm run build && npm run db:migrate) fallo con 'nodenv: command not found' — wrapper interno de Plesk al fijar la version de Node, NO es nuestro script ni un hook del repo — asi que esos pasos se ejecutaron manualmente desde Node.js > 'Run Node.js commands'. (2) HALLAZGO CRITICO corregido en caliente: 'Document Root' del panel Node.js apuntaba a /httpdocs/dist (rompe /api, /uploads y el fallback SPA al saltarse Express) -> corregido a /httpdocs. (3) 'npm ci' OK (606 paquetes, incluye vite/sharp). (4) 'npm run db:migrate' fallo dos veces por CONFIGURACION del servidor, no por codigo: (a) las 'Custom environment variables' del panel Node.js NO se inyectan en el contexto de 'Run Node.js commands' -> hubo que crear manualmente un archivo .env en /httpdocs (gitignored; cargado por dotenv via server/config/env.js) con los mismos valores; (b) el valor configurado DB_NAME=crudo_dev (heredado de .env.example, plantilla de dev) NO coincidia con el nombre real de la base de datos creada en Plesk: 'crudo_v1' (el usuario crudo_user esta vinculado a crudo_v1, no a crudo_dev) -> corregido DB_NAME=crudo_v1 tanto en .env como en el panel Custom environment variables. Tras el fix: db:migrate aplico 4/4 migraciones (001_create_core_schema, 002_create_indexes, 003_add_pickup_idempotency, 004_admin_security_and_config), 0 omitidas. (5) 'npm run build' OK (1754 modulos, dist/ generado, bundle index 255KB / gzip 78KB, sin warnings). (6) 'npm prune --omit=dev' OK (-468 paquetes dev). (7) Restart App + dominio activado (estaba 'Disabled' intencionalmente, pre-lanzamiento). PENDIENTES post-deploy anotados para el owner (NO bloquean el cierre de Fase 1, no se actuo sobre ellos en esta sesion): (a) ROTAR SECRETOS — DB_PASSWORD, JWT_SECRET y COOKIE_SECRET quedaron visibles en texto plano durante el diagnostico (capturas del panel Plesk); rotar y actualizar en .env + panel Node.js + Restart App; (b) revisar 'npm audit' — 10 vulnerabilidades antes de prune (9 moderate/1 critical en devDeps), 4 moderate quedan en runtime de produccion; (c) version de Node del servidor es 26.3.0 (no Node 20 LTS como asumia la doc previa) — funciono sin incompatibilidades observadas, pero revisar/actualizar infra/plesk/README.md si se confirma asi; (d) actualizar infra/scripts/deploy-checklist.md e infra/plesk/README.md con: nombre real de la BD de produccion (crudo_v1, no crudo_dev de la plantilla), necesidad de un .env fisico en /httpdocs ademas de las Custom environment variables (porque 'Run Node.js commands' no las hereda), y el flujo 100% UI sin SSH que SI funciono aqui: Git pull/deploy -> revisar/corregir Document Root -> Node.js > Run Node.js commands (npm ci / run db:migrate / run build / prune --omit=dev) -> Restart App -> activar dominio -> smoke.sh. Fases previas: 0,1,2,3,4,5,6,7,8.\nNOTA Fase 8 ejecutada. Cookies/consent YA existian (consent.js modelo AEPD + CookieBanner accept/reject/configure; analytics consent-aware). Añadido: public/robots.txt (disallow /admin) + public/sitemap.xml (11 rutas, dominio crudomov.es a confirmar). Nuevo src/hooks/useSeo.js (sin react-helmet: title+description+canonical+OG+Twitter por ruta; SITE_URL via VITE_SITE_URL default crudomov.es) aplicado a Home/Seleccion/Eventos/EventDetail/Tablas/Merch/Contacto/Legal. index.html: OG por defecto + canonical + JSON-LD Store (LocalBusiness, NO wine bar, horarios reales). Nuevo EventJsonLd.jsx: schema.org/Event en detalle con offer (sin pago online). GA4 conectado en analytics.js: gtag se carga PEREZOSO solo si VITE_GA_ID existe Y hay consentimiento analytics (anonymize_ip, allow_google_signals=false, sin PII); si no hay ID, no-op. LegalPage: datos confirmados (CRUDO QUESOS S.L.U, CIF B-19953694, dir.) + aviso +18/no venta alcohol online/no pago online; marcado como pendiente de validacion legal del titular. Verificado: lint limpio, test:client 90/90, build OK, robots+sitemap copiados a dist. Pendiente owner: GA4 ID, decision Meta Pixel, validacion legal final, confirmar dominio .es vs .com. Fases previas: 0,2,3,4,5,6,7.\nNOTA Fase 7: panel admin mobile-first panel admin mobile-first sobre /api/v1/admin/**. /admin deja de ser placeholder (AdminEntryPage.jsx eliminado). Nuevos: src/lib/adminApi.js (axios base /api/v1/admin, Authorization Bearer, refresh automatico en 401 una vez, tokens en localStorage con riesgo XSS documentado), src/hooks/useAdminAuth.jsx (context login/logout/persistencia), components/admin/AdminShell.jsx (layout movil + bottom-nav 4 tabs, targets >=44px), components/admin/RequireAdmin.jsx (guard → redirige a /admin si no auth). Paginas: AdminLoginPage (email/password, contrato access_token/refresh_token/admin), AdminDashboardPage (pickups hoy, eventos proximos, consultas nuevas, alertas disponibilidad, pedidos nuevos), AdminProductsPage (lista + buscador + toggle disponibilidad IN_STOCK/LOW/OUT via PATCH /products/:id/stock, usa availability.js, wine guard VISIBLE 'Vino · solo WhatsApp'), AdminEventsPage (lista + estado publicado/oculto), AdminOrdersPage (lista + flujo de estado NEW→CONFIRMED→READY→PICKED_UP / CANCELLED via PATCH). Rutas anidadas /admin/* en routes.jsx (chunks admin 3-6KB separados, no afectan bundle publico). Verificado: lint limpio, test:client 90/90, build OK sin warning. Smoke admin en navegador PENDIENTE de validar por usuario (necesita backend+DB+seed). NO implementado en esta fase (cola Fase 7+): CRUD completo crear/editar producto y evento, subida de imagenes, consultas/newsletter UI, ajustes (pickup paused/capacidad). El admin actual cubre las acciones diarias criticas. Fases previas: 0,2,3,4,5,6.\nNOTA Fase 6: ejecutada. Hallazgo: el modelo YA era editorial Hallazgo: el modelo YA era editorial (no hay inventario por unidades en ningun sitio). DB usa stock_status enum (IN_STOCK/LOW/OUT) + is_active para ocultar; catalogo publico ya filtra is_active=1; validador admin acepta stock_status + is_active sin pedir cantidades; no hay texto 'stock' visible al cliente (solo identificadores de codigo). No hace falta migracion. Entregables: nuevo src/lib/availability.js como fuente unica de copy editorial (Disponible/Pocas unidades/Agotado + Oculto via is_active; tonos; OWNER_AVAILABILITY_OPTIONS para admin Fase 7). StockBadge refactorizado para usar availability.js (sin cambio de comportamiento). docs/owner-admin-guide.md: seccion reescrita 'Disponibilidad de un producto (no es inventario)' con tabla de 4 estados. Mi Tabla intacto (precios desde DB; alcohol guard backend sin tocar). Verificado: lint limpio, test:client 90/90, build OK; alcohol guard = tests server (necesitan DB, no ejecutados local pero backend no modificado). Fases previas: 0,2,3,4,5.\nNOTA Fase 5: ejecutada (alcance acotado a consumo de assets V2 + alineacion de componentes clave, sin reescritura total). Nuevo componente ResponsiveImage (src/components/ui/ResponsiveImage.jsx) que sirve WebP responsive del manifest v2Assets con srcSet/sizes y art-direction opcional (mobile distinto). Conectado en pantallas prioritarias: Hero home (picture art-directed: movil 4:5 / desktop 16:10, eager), TablasPage hero (tablas-hero), EventsPage hero (eventos-hero), ContactPage hero (lifestyle-bodegon), CategoryStrips home (3 strips → seleccion-hero/lifestyle-bodegon/eventos-hero con srcSet), EventDetail FALLBACK_HERO (eventos-hero), ProductCard fallbacks 1:1 (fallback-queso/fallback-maridaje con srcSet) + hover lift del diseño. Enlaces internos primarios apuntan a /seleccion (Home primaryHref + Hero default). ProductCard ya respetaba regla alcohol (vino→WhatsApp, no-alcohol→Añadir). Bundle: vendor-ui nuevo (cva+clsx+tailwind-merge 21KB) → index 479.57KB (gzip 131KB), warning eliminado. Test ProductCard actualizado a nuevos alt del manifest. Verificado: lint limpio, test:client 90/90, build OK sin warning. Smoke visual en navegador PENDIENTE de validar por usuario (npm run dev; backend necesita DB). Pendiente Fase 5 ampliada (futuro): alinear fino Footer/ContactPage layout/eyebrow-pill al mockup; no critico. Fases previas: 0,2,3,4.\nNOTA Fase 4: pipeline de imagenes con sharp (devDependency, solo build-time). Script idempotente scripts/build-images.mjs (npm run build:images) config-driven: 11 roles curados desde fotos alta-res ya en repo, recortados por ratio con gravedad attention (recorte inteligente del sujeto). Genera 27 WebP responsive en public/img/v2/: home-hero (16:10 768/1200/1800) + home-hero-mobile (4:5 480/768), seleccion-hero (16:9), tablas-hero (16:10), eventos-hero (16:9), evento-poster-telperion (3:4 480/768/1000), contacto-local (16:9), fallbacks 1:1 queso/maridaje/tabla (400/800), lifestyle-bodegon. Total v2 = 1.0MB (vs 12MB originales); mayor archivo 92KB (vs 3.3MB originales about/*.jpg). Manifest autogenerado src/lib/v2Assets.js con {alt, ratio, width, height, src, srcSet} por rol — listo para consumir en Fase 5 (aun NO referenciado por componentes). Originales V1 intactos (no se borra nada). Verificado: build:images OK, manifest valido (11 roles), lint limpio, test:client 90/90, build OK sin warning. Fases previas: 0 auditoria, 3 code splitting, 2 intake diseño (spec seccion 17)."
next_recommended_prompt: "Fase 9 REVIEW_READY — suite E2E (Playwright, API mockeada) + a11y (axe) + check:bundle + qa-checklist, todo verde local (lint, test:client 90/90, build, check:bundle, test:e2e 58/58). Validacion humana pendiente para pasar Fase 9 a DONE: correr 'npm run test:e2e' y QA manual de docs/qa-checklist.md (iPhone Safari, Instagram in-app, Lighthouse mobile). Siguiente fase recomendada: Fase 10 (limpieza de codigo/repo, seccion 12) o Fase 11 (launch). DECISION DE DISEÑO pendiente: contraste de marca < AA (acento rosa sobre crema), ver hallazgo en current_focus y qa-checklist.md. Pendientes Fase 1 para owner (no bloquean): ROTAR SECRETOS expuestos (DB_PASSWORD/JWT_SECRET/COOKIE_SECRET), revisar npm audit, actualizar infra/plesk/README.md + deploy-checklist.md con DB_NAME real=crudo_v1, .env fisico en servidor y flujo deploy 100% UI sin SSH. Colas: alineacion fina Footer/Contact (Fase 5); CRUD completo + imagenes + ajustes admin (Fase 7)."
...
| 1 | Produccion/Plesk y deploy reproducible | DONE | Verificar crudomov.es, env, DB, build, migraciones, SSL y smoke. (CERRADA 2026-06-08: deploy ejecutado en produccion via Plesk UI sin SSH; smoke.sh -> ok=9 fail=0. Hallazgos corregidos en caliente: Document Root /httpdocs/dist→/httpdocs, DB_NAME real=crudo_v1 (no crudo_dev de la plantilla), .env fisico creado en servidor porque las Custom environment variables de Plesk no llegan a 'Run Node.js commands'. Pendiente no bloqueante: rotar DB_PASSWORD/JWT_SECRET/COOKIE_SECRET expuestos durante el diagnostico — ver nota detallada en current_focus.) |
...
| 9 | E2E, accesibilidad, performance y QA | REVIEW_READY | Playwright, Lighthouse, checks mobile y regresiones criticas. (Hecho: Playwright API-mockeada 58/58 mobile+desktop, 8 flujos criticos + alcohol guard frontend; axe a11y estructural; scripts/check-bundle.mjs; docs/qa-checklist.md; job e2e en CI. Hallazgo no bloqueante: contraste de marca < AA, decision de diseño pendiente. QA manual real y Lighthouse pendientes de owner para DONE.) |
